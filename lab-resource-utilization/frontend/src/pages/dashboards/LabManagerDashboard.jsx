...
const LabManagerDashboard = () => {
  ...
  const dashboardItems = [
    { 
      title: "Department equipment utilization heatmap", 
      desc: "Visualize usage patterns across your department's resources.", 
    ...
    { 
      title: "Booking adoption and no-show rates", 
      desc: "Track how effectively your lab's booking system is being used. Click to view all bookings.", 
      icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z", 
      color: "text-emerald-400", bg: "bg-emerald-500/10",
      value: analytics ? analytics.totalBookings : null,
      label: "Total Bookings",
      link: "/bookings"
    },
    { 
      title: "Maintenance schedule overview", 
      desc: "Keep track of upcoming maintenance and repairs for your equipment. Click to view inventory.", 
    ...
    { 
      ...
      desc: "Manage inbound equipment sharing requests. Click to view and approve.", 
      icon: "M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4", 
      color: "text-purple-400", bg: "bg-purple-500/10",
      value: pendingBookingsCount,
      label: "Pending Approvals",
      link: "/bookings?tab=all&status=PENDING_APPROVAL"
    }
  ];
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dashboardItems.map((item, idx) => (
            <div 
              ...
              className={`bg-[#12131a] border border-white/[0.05] rounded-2xl p-6 shadow-xl transition flex flex-col h-full relative overflow-hidden group 
                ${item.link ? 'hover:-translate-y-1 hover:border-purple-500/30 cursor-pointer' : ''}`}
            >
              {item.isUnderDev && (
              ...
              <div className={`mt-auto h-24 flex items-center justify-center border border-dashed border-white/[0.1] rounded-lg text-gray-500 text-sm transition-all ${item.link ? 'group-hover:border-purple-500/20' : ''}`}>
                 {loading ? (
                    ...
                    <span className={`flex items-center gap-2 px-4 py-2 rounded-lg ${item.link ? 'group-hover:bg-purple-500/5' : ''}`}>
                      <svg className="w-4 h-4 animate-spin text-purple-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                      Coming Soon
                    </span>
                 ) : (
                    <div className={`text-center transition-transform ${item.link ? 'group-hover:scale-105' : ''}`}>
